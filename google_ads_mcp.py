...
Conecta no Claude.ai via Settings → Integrations → Add MCP Server.
"""
...
# ─── Credenciais (mesmas env vars do Railway) ────────────────────────────────

GOOGLE_ADS_DEVELOPER_TOKEN = os.environ.get("GOOGLE_ADS_DEVELOPER_TOKEN", "")
GOOGLE_CLIENT_ID           = os.environ.get("GOOGLE_CLIENT_ID", "")
GOOGLE_CLIENT_SECRET       = os.environ.get("GOOGLE_CLIENT_SECRET", "")
GOOGLE_REFRESH_TOKEN       = os.environ.get("GOOGLE_REFRESH_TOKEN", "")
MCP_SECRET                 = os.environ.get("MCP_SECRET", "")  # token de autenticação
...
def _query(customer_id, gaql, login_customer_id=None):
    token = _access_token()
    headers = {
...
def _resolve_customer_id(raw: str) -> str:
    """Aceita nome do cliente (oimu, kukiê…) ou ID numérico direto."""
    key = raw.strip().lower()
    if key in CUSTOMER_IDS:
        return CUSTOMER_IDS[key]
...
async def list_tools():
    return [
        Tool(
            ...
            description=(
                "Retorna performance por campanha de uma conta Google Ads. "
                "Aceita nome do cliente (ex: 'oimu', 'kukiê', '787') ou customer_id numérico."
            ),
            inputSchema={
                "type": "object",
                "properties": {
                    "customer": {
                        "type": "string",
                        "description": "Nome do cliente (oimu/kukiê/infanti/vj/undertop/787/monnari/cora) ou customer_id numérico",
                    },
                    "period": {
                        "type": "string",
                        "enum": ["this_month", "last_7_days", "last_30_days", "last_month"],
        ...
        Tool(
            ...
            description=(
                "Retorna resumo consolidado (gasto total, conversões, ROAS) de uma conta Google Ads. "
                "Aceita nome do cliente ou customer_id numérico."
            ),
            inputSchema={
                "type": "object",
                "properties": {
                    "customer": {
                        "type": "string",
                        "description": "Nome do cliente ou customer_id numérico",
                    },
                    "period": {
                        "type": "string",
                        "enum": ["this_month", "last_7_days", "last_30_days", "last_month"],
...
async def call_tool(name: str, arguments: dict):
    ...
    customer_id = _resolve_customer_id(arguments.get("customer", ""))
